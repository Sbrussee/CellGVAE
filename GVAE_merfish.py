...
def apply_on_dataset(model, dataset, name, celltype_key, args, discriminator=None):
    ...
    sc.pl.spatial(dataset, layer='X', spot_size=0.02, color=['total_counts'],
    ...
    sc.pl.spatial(dataset, layer='pred', spot_size=0.02, color=['total_pred'],
    ...
    sc.pl.spatial(dataset, layer='error', spot_size=0.02, title='Spatial distribution of total prediction error',
                  save=f"total_error_spatial_{name}.png", color=['total_error'], show=False)
    plt.close()
    sc.pl.spatial(dataset, layer='error', spot_size=0.02, title='Spatial distribution of relative prediction error',
    ...
    for gene in dataset.var_names:
        sc.pl.spatial(dataset, use_raw=False, color=[gene], spot_size=0.02,
...
def spatial_analysis(adata, celltype_key, name):
    ...
    """
    sc.pl.spatial(adata, use_raw=False, spot_size=0.02, title=f'Spatial celltype distribution',
