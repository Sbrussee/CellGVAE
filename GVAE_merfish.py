...
def plot_latent(model, pyg_graph, anndata, cell_types, device, name, number_of_cells, celltype_key, args, plot_celltypes=False):
    ...
    plot = sns.scatterplot(x=tsne_z[:,0], y=tsne_z[:,1], s=0.05, hue=list(anndata[:number_of_cells,:].obs[celltype_key]))
    ...
    plot = sns.scatterplot(x=umap_z[:,0], y=umap_z[:,1], s=0.05, hue=list(anndata[:number_of_cells,:].obs[celltype_key]))
    plot.legend(fontsize=3)
    ...
    plot = sns.scatterplot(x=transformed_data[:,0], y=transformed_data[:,1], s=0.05, hue=list(anndata[:number_of_cells,:].obs[celltype_key]))
    plot.legend(fontsize=3)
    plt.xlabel("PC1")
    plt.ylabel("PC2")
...
def apply_on_dataset(model, dataset, name, celltype_key, args, discriminator=None):
    ...
    sc.pl.spatial(dataset, layer='X', spot_size=0.05, color=['total_counts'],
    ...
    sc.pl.spatial(dataset, layer='pred', spot_size=0.05, color=['total_pred'],
    ...
    sc.pl.spatial(dataset, layer='error', spot_size=0.05, title='Spatial distribution of total prediction error',
                  save=f"total_error_spatial_{name}.png", color=['total_error'], show=False)
    plt.close()
    sc.pl.spatial(dataset, layer='error', spot_size=0.01, title='Spatial distribution of relative prediction error',
    ...
    for gene in dataset.var_names:
        sc.pl.spatial(dataset, use_raw=False, color=[gene], spot_size=0.05,
                      title=f'Spatial distribution of predicted expression of {gene}',
                      save=f"predicted_expr_spatial_{name}_{gene}.png", show=False)
        plt.close()
        sc.pl.spatial(dataset, layer='error', color=[gene], spot_size=0.05,
    ...
    relative_error_per_gene = total_error_per_gene / np.reshape(sum_x, (-1,1))
...
def spatial_analysis(adata, celltype_key, name):
    ...
    """
    sc.pl.spatial(adata, use_raw=False, spot_size=0.05, title=f'Spatial celltype distribution',
