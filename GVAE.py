...
def plot_latent(model, pyg_graph, anndata, cell_types, device, name, number_of_cells, celltype_key, args, plot_celltypes=False):
    ...
    plot = sns.scatterplot(x=tsne_z[:,0], y=tsne_z[:,1], s=0.5, hue=list(anndata[:number_of_cells,:].obs[celltype_key]))
    ...
    plot = sns.scatterplot(x=umap_z[:,0], y=umap_z[:,1], s=0.5, hue=list(anndata[:number_of_cells,:].obs[celltype_key]))
    plot.legend(fontsize=3)
    ...
    plot = sns.scatterplot(x=transformed_data[:,0], y=transformed_data[:,1], s=0.5, hue=list(anndata[:number_of_cells,:].obs[celltype_key]))
    plot.legend(fontsize=3)
    plt.xlabel("PC1")
    plt.ylabel("PC2")
...
def train(model, pyg_graph, optimizer_list, train_i, val_i, k, args, discriminator=None, dataset=None):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if args.innerproduct:
            ...
            A_hat = ipd.forward(torch.from_numpy(get_latent_space_vectors(model, pyg_graph.to(device), dataset, device, args=args)).to(device))
            A = torch.from_numpy(to_scipy_sparse_matrix(pyg_graph.edge_index).toarray())
            total_loss_over_cells += np.mean((A - A_hat.cpu()) ** 2)
        batch = batch.cpu()
