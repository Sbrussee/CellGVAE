...
def plot_latent(model, pyg_graph, anndata, cell_types, device, name, number_of_cells, celltype_key, args, plot_celltypes=False):
    ...
    plot = sns.scatterplot(x=tsne_z[:,0], y=tsne_z[:,1], s=0.1, hue=list(anndata[:number_of_cells,:].obs[celltype_key]))
    ...
    plot = sns.scatterplot(x=transformed_data[:,0], y=transformed_data[:,1], s=0.1, hue=list(anndata[:number_of_cells,:].obs[celltype_key]))
    plot.legend(fontsize=3)
    plt.xlabel("PC1")
    plt.ylabel("PC2")
